...
controllerProductos.crearProductos=async function(req,res){
    ...
    res.send({title:"ok",message:"el producto a sido agregado sastifactoriamente"})
}
...
controllerProductos.eliminarProductos=function(req,res){
    ProductosSchemas.deleteOne({_id:req.params.id},function(err){
        if(err) res.send({title:"error",message:"el producto es inexistente"})
        res.send({title:"ok",message:"el producto ha sido eliminado"})
    })
}
